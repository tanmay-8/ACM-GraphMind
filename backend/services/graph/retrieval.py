...
class GraphRetrieval:
    ...
    def retrieve(
        ...
        try:
            with self.driver.session() as session:
                ...
                print(f"[Retrieval] Primary mode: {mode.value}, depth: {recommended_depth}")
                ...
                # 5. Calculate real hop distances from User node
                nodes_with_hops = self._calculate_hop_distances(
                    session, user_id, raw_nodes
                )
                
                # 6. Apply advanced scoring and ranking
                retrieved_nodes = self._score_and_rank_nodes_advanced(
                    nodes_with_hops, query, centrality_scores, user_id
                )
    ...
    def _execute_ensemble_retrieval(
        ...
        for mode in RetrievalMode:
            try:
                mode_nodes = self._execute_mode_based_retrieval(
                    session, user_id, mode, start_date, depth
                )
        ...
        return ensemble_nodes[:200]  # Return top 200 after ensemble fusion
